...
meancomp <- t(data.matrix(colMeans((complete_MAR5_ridge), na.rm=T)))%>% round(.,3) %>% as.data.frame(.,) %>% mutate(cond = "no missingness", model = "LASSO", mis_prop = 0.25)
...
meancomp <- t(data.matrix(colMeans((complete_MAR7_ridge), na.rm=T)))%>% round(.,3) %>% as.data.frame(.,) %>% mutate(cond = "no missingness", model = "LASSO", mis_prop = 0.75)
