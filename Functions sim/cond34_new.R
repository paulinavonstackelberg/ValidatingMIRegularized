...
#  models <- list()
  ...
  for (fold in 1:k_def){
    ...
    imp <- mice(stacked, maxit = 15, method = "pmm", ignore = ig, print = F)
...
#  models <- list()
  ...
  for (fold in 1:k_def){
    ...
    imp_train <- mice(train, maxit = 15, method = "pmm",  print = F)
    imp_test <- mice(test, maxit = 15, method = "pmm", print = F)
