...
cond_2y <- function(data, model, thresh, num_param, true_param, k_def){
 ...
 # models <- list()
  ...
  for (fold in 1:k_def){
    ...
    imp_stacked <- mice(stacked, maxit = 15, method = "pmm", print = F)
    ...
    # to predict each test set model (-> 5*5 =25 predictions)
    ...
    #test_sets[[1]] == test_sets[[2]]
    # replace the imputed y in the test sets with the real y
    
    #test_sets[["1"]][["y"]] == test_y
    test_sets <- lapply(X = test_sets, FUN = replace_y, y_real = test_y, num_param = num_param)  
    
    # test_sets[[1]][,21] == test_sets[[2]][,21]
    #test_sets[[1]][,21] == test_sets[[3]][,21]
    #test_sets[[1]][,21] == test_sets[[4]][,21]
    #test_sets[[2]][,21] == test_sets[[3]][,21]
    #test_sets[[2]][,21] == test_sets[[4]][,21]
    #test_sets[[2]][,21] == test_sets[[5]][,21]
